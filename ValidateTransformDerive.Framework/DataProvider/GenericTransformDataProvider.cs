...
namespace ValidateTransformDerive.Framework.DataProvider
{
    ...
    {
        protected readonly ITransformationService<TSource, TDest> TransformationService;
        ...
        public GenericTransformDataProvider(ITransformationService<TSource, TDest> transformationService, IProvideData<TSource> dataProvider)
        {
        ...
        public virtual async Task<IEnumerable<TDest>> GetAllAsync(CancellationToken cancellationToken = default)
            => (await DataProvider.GetAllAsync(cancellationToken)).Select(item => TransformationService.Transform(item));
    }
}
